<!-- Bootstrap core CSS -->
  <link href="http://getbootstrap.com/dist/css/bootstrap.min.css" rel="stylesheet">   
<?php
   echo $this->Html->css('dashboard2');
   ?>
	
	  <div class="container-fluid">
      <div class="row">
        

          <h1 class="page-header">MUve Dashboard</h1>

          <div class="row placeholders">
            <div class="col-xs-6 col-sm-3 placeholder">
			<!-- $this-html-image for images  -->
			echo $this->Html->image("dashExercise.jpg", array(
    "alt" => "Exercise Log",
    'url' => array('controller' => 'users', 'action' => 'view', exercise)
																));
              <img data-src="holder.js/200x200/auto/sky" class="img-responsive" alt="200x200" src="data:image/svg+xml;base64,">
              <h4>Group Challenges</h4>
              <span class="text-muted">Challenges</span>
            </div>
            <div class="col-xs-6 col-sm-3 placeholder">
              <img data-src="holder.js/200x200/auto/vine" class="img-responsive" alt="200x200" src="data:image/svg+xml;base64,">
              <h4>Group Achievements</h4>
              <span class="text-muted">Group Achievements</span>
            </div>
            <div class="col-xs-6 col-sm-3 placeholder">
              <img data-src="holder.js/200x200/auto/sky" class="img-responsive" alt="200x200" src="data:image/svg+xml;base64,">
              <h4>Group Members</h4>
              <span class="text-muted">View Group Members</span>
            </div>
			
			<!-- ONLY DISPLAY IF THEY ARE ADMINS... THIS IS WHERE THEY WILL EDIT GROUP MEMBERS/PRIVLIDGES -->
			
			<div class="col-xs-6 col-sm-3 placeholder">
              <img data-src="holder.js/200x200/auto/sky" class="img-responsive" alt="200x200" src="data:image/svg+xml;base64,">
              <h4>Edit Members</h4>
              <span class="text-muted">Edit Users</span>
            </div>
          </div>
		<!-- PHP FOREACH LOOP TO DISPLAY WHATEVER IT IS ON EACH DASHBOARD ITEM. Needed on overview? Probably not. -->
          <h2 class="sub-header">Group Members</h2>
          <div class="table-responsive">
            <table class="table table-striped">
              <thead>
                <tr>
                  <th>Name</th>
                  <th>Join Date</th>
                  <th>Description</th>
                </tr>
              </thead>
              <tbody>               
                <tr>
                  <td>Doge</td>
                  <td>3/20/2014</td>
                  <td>such join</td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
      </div>
    </div>
